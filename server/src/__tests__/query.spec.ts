...
describe('User actions: follow, items, ratings, search friends', () => {
  ...
  it('should register two users and allow follow/unfollow', async () => {
    ...
    expect(Array.isArray(followRes.body.followers)).toBe(true);
    expect(followRes.body.followers.map((s: any) => String(s))).toContain(String(id1));
    ...
    expect(unf.body.followers.map((s: any) => String(s))).not.toContain(String(id1));
  });
