            ...
            <div v-if="showFilters" ref="filtersPanel" class="mt-4 z-50 w-full max-w-[760px] p-6 mx-auto rounded-xl shadow-2xl border border-slate-700 bg-slate-900/90 transition-all duration-200">
              <div class="flex items-center gap-4">
                ...
                <div class="ml-auto flex gap-2">
                  <button @click="applyFilters" class="px-4 py-2 rounded-full bg-gradient-to-r from-primary-500 to-accent-500 text-white font-semibold hover:brightness-110 shadow-glow transition-all">Aplicar</button>
                  <button @click="resetFilters" class="px-4 py-2 rounded-full bg-gradient-to-r from-primary-500 to-accent-500 text-white font-semibold hover:brightness-110 shadow-glow transition-all">Reset</button>
                </div>
              </div>
            </div>
          </div>
              ...
              <li v-for="(u, idx) in results" :key="u._id" class="py-0">
                <div class="flex items-center justify-between p-4 rounded-md bg-slate-900/80 border border-slate-700 shadow-sm">
                  ...
                  <div class="flex items-center">
                    ...
                    <button v-else @click="handleFollow(u._id, idx)" class="w-10 h-10 flex items-center justify-center rounded-full bg-gradient-to-r from-primary-500 to-accent-500 text-white hover:brightness-110 shadow-glow transition-all">
                      <!-- user-plus icon -->
                      <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
